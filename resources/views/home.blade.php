...
@section('content')
    <div class="container">
        @if (Session::has('review'))
            <p class="alert {{ Session::get('alert-class', 'alert-success') }}"> {{ Session::get('review') }}</p>
        @endif
    </div>
        ...
        <div class="container my-5 py-5">
            <div class="row align-items-center g-5">
                <div class="col-lg-6 text-center text-lg-start">
                    ...
                    <a href="{{ url(route('home') . '#bookTable') }}"
            ...
            <div class="tab-class text-center wow fadeInUp" data-wow-delay="0.1s">
                ...
                <div class="tab-content">
                    ...
                    <div id="tab-2" class="tab-pane fade show p-0">
                        <div class="row g-4">
                            @foreach ($lunchFoods as $lFood)
                                <div class="col-lg-6">
                                    <div class="d-flex align-items-center">
                                        <img class="flex-shrink-0 img-fluid rounded"
                                            src="{{ asset('assets/img/' . $lFood->image . '') }}" alt=""
                                            style="width: 80px;">
                                        <div class="w-100 d-flex flex-column text-start ps-4">
                                            <h5 class="d-flex justify-content-between border-bottom pb-2">
                                                <span>{{ $lFood->name }}</span>
                                                <span class="text-primary">{{ $lFood->price }}</span>
                                            </h5>
                                            <small class="fst-italic">{{ substr($lFood->description, 0, 70) }}</small>
                                            <a type="button" href="{{ route('food.details', $lFood->id) }}"
                                                class="btn btn-primary py-2 top-0 end-0 mt-2 me-2">view</a>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div id="tab-3" class="tab-pane fade show p-0">
                        <div class="row g-4">
                            @foreach ($dinnerFoods as $dFood)
                                <div class="col-lg-6">
                                    <div class="d-flex align-items-center">
                                        <img class="flex-shrink-0 img-fluid rounded"
                                            src="{{ asset('assets/img/' . $dFood->image . '') }}" alt=""
                                            style="width: 80px;">
                                        <div class="w-100 d-flex flex-column text-start ps-4">
                                            <h5 class="d-flex justify-content-between border-bottom pb-2">
                                                <span>{{ $dFood->name }}</span>
                                                <span class="text-primary">{{ $dFood->price }}</span>
                                            </h5>
                                            <small class="fst-italic">{{ substr($dFood->description, 0, 70) }}</small>
                                            <a type="button" href="{{ route('food.details', $dFood->id) }}"
                                                class="btn btn-primary py-2 top-0 end-0 mt-2 me-2">view</a>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="container">
        @if (Session::has('booking-error'))
            <p class="alert {{ Session::get('alert-class', 'alert-danger') }}"> {{ Session::get('booking-error') }}</p>
        @endif
    </div>

    {{-- @if ($errors->any())
{{ implode('', $errors->all('<p class="alert '. Session::get('alert-class', 'alert-danger') .'"> :message</p>')) }}
    ...
    @endif
    @if ($errors->has('date'))
        <p class="alert {{ Session::get('alert-class', 'alert-danger') }}"> {{ $errors->first('date') }}</p>
    @endif
    @if ($errors->has('num_people'))
        <p class="alert {{ Session::get('alert-class', 'alert-danger') }}"> {{ $errors->first('num_people') }}</p>
    @endif
    @if ($errors->has('spe_request'))
        <p class="alert {{ Session::get('alert-class', 'alert-danger') }}"> {{ $errors->first('spe_request') }}</p>
    @endif

    <!-- Reservation Start -->
    <div id="bookTable" class="container-xxl py-5 px-0 wow fadeInUp" data-wow-delay="0.1s">
        <div class="row g-0">
            <div class="col-md-6">
                <div class="video">
                    ...
                    <div class="testimonial-item bg-transparent border rounded p-4">
                        ...
                        <div class="d-flex align-items-center">
                            <img class="img-fluid flex-shrink-0 rounded-circle"
                                src="{{ asset('assets/img/' . $testimonial->image) }}"
